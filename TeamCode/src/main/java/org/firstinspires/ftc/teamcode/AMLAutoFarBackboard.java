...
public class AMLAutoFarBackboard extends LinearOpMode implements VisionPortalUser, TensorflowProp, UsesTrajectories{
        ...
        if (!isStopRequested()) {
            if (!turnTest) {
                ...
                if (pos == ProcessDetections.pos.right) {
                    ...
                    drive.followTrajectorySequence(trajs.get(trajNames.moveUnderTruss));
    ...
    public TrajectorySequence backboardSetupRightFar(SampleMecanumDrive drive) {
        TrajectorySequence backboardSetupRightFar = drive.trajectorySequenceBuilder(curPose)
                .strafeRight(12)
                .lineToLinearHeading(new Pose2d()) //unfinished
                .build();
        trajs.put(trajNames.moveUnderTruss, backboardSetupRightFar);
        switchCurPose(backboardSetupRightFar.end());
        return backboardSetupRightFar;
    }
    ...
    public void buildInitialTrajs(SampleMecanumDrive drive)
    {
        ...
        switchCurPose(new Pose2d(0 ,0, Math.toRadians(90)));
